...
**Known, explained, deliberately-not-fixed-yet limitation**: `notFound()` (e.g. visiting `/matches/<bad-id>` or `/wallets/<bad-address>`) renders the correct not-found content in the body, but returns HTTP 200 instead of 404 — **only in a production build** (`next build && next start`); `next dev` gets this right. Initially spent a while empirically ruling out `loading.tsx`, `error.tsx`, and `export const dynamic = "force-dynamic"` one at a time (each really did get tested before/after against a real production server) before finding the actual documented explanation in Next's own bundled docs (`node_modules/next/dist/docs/01-app/03-api-reference/04-functions/not-found.md`, "Calling notFound() after streaming has started"):

> "Because the check runs inside the `<Suspense>` boundary, the response has already begun streaming as a `200`, and the status can't change once streaming has started. [...] To return a real `404` status, the resource has to be checked before the response streams. [...] run that check in `proxy` instead."

So this isn't a bug in this codebase — dynamic route segments get wrapped in an implicit Suspense boundary in production regardless of an explicit `loading.tsx` (removing every `loading.tsx`/`error.tsx` in the app didn't change it, confirming the boundary isn't coming from application code), and the docs confirm the status code fundamentally cannot change after that point. The documented fix is a root `proxy.ts` (Next 16's rename of Middleware) that checks existence and returns a real 404 *before* the route even starts rendering — **not implemented**, since it would mean duplicating the existence-check query outside the page component for a personal, single-user, not-yet-public tool where getting a real HTTP 404 status only matters for SEO/tooling, not for what the one user actually sees (the content is already correct). Revisit if/when this ever needs to be crawler- or tool-facing.
